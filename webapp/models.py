...
class DeadlineOverride(Base):
    ...
    student = sa.Column("student", sa.Integer, sa.ForeignKey("students.id"), nullable=False, primary_key=True)
    block = sa.Column("block", sa.Integer, sa.ForeignKey("task_blocks.id"), nullable=False, primary_key=True)
    deadline = sa.Column("deadline", sa.DateTime, nullable=False)
    reason = sa.Column("reason", sa.String, nullable=False)
    teacher = sa.Column("teacher", sa.Integer, sa.ForeignKey("students.id"), nullable=False)
